...
describe("buildEventRelation — spec worked example", () => {
  const r = buildEventRelation({ pA: 0.18, pB: 0.22, estimateRho: 0.45, labelA: "France win WC", labelB: "Mbappe golden boot" });
  ...
  test("same_exposure signal, reverse hedge ratio ≈ −0.42, effectiveness ≈ 0.20", () => {
    ...
    expect(r.reasoning).toContain("share the same exposure");
  });
});

describe("structural relations (path A, high confidence)", () => {
  ...
  test("containment (subset) → strong positive φ, same_exposure, not a hedge", () => {
    const r = buildEventRelation({ pA: 0.124, pB: 0.45, structuralJoint: 0.124, structuralKind: "subset", labelA: "England win WC", labelB: "Europe win WC", liquidityOk: true });
    expect(r.relation).toBe("related");
